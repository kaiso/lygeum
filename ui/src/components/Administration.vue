...
<template>
  <aps-layout :title="$t('layout.administration')">
<v-container slot="mainContent" grid-list-md text-xs-center style="height:100%">
    <v-layout row wrap>
      <v-flex xs6>
        <v-flex xs12>
          <v-avatar color="brown lighten-3" size="100" @click.stop="route('/admin/users')"
          style="cursor:pointer;">
            <v-btn icon>
            <v-icon dark size="70">people</v-icon>
            </v-btn>
          </v-avatar>
        </v-flex>
      <label>{{$tc('admin.user', 2)}} </label>
      </v-flex>
      <v-flex xs6>
        <v-flex xs12>
